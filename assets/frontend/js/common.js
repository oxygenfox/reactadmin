...
jQuery(document).ready(function() {
    var data = {order_id:"", name:"", startDate: "", endDate: ""};
...
jQuery(document).on('click','#filter-order-filter', function(){
    ...
    var data = {order_id:order_id, name:name, startDate:startDate, endDate:endDate};
    generateOrderTable(data);
});
...
function generateOrderTable(element){	
    jQuery.ajax({
        ...
        data: {'order_id' : element.order_id , 'name' : element.name, 'start_date' : element.startDate , 'end_date' : element.endDate},
        type: 'post', 
        dataType: 'json',
        beforeSend: function () {
